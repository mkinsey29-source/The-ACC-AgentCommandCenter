...
Two Gemini drivers land the same way the DeepSeek ones did: a plain-API driver plus a CLI-wrapper driver, both verified against Google's own current sources rather than assumed continuity with older, pre-2026 Gemini API knowledge. A `driver: 'gemini'` agent type (`acc/gemini.py`, mirroring `ollama.py`) calls the plain developer-facing Interactions API directly over HTTP (`POST generativelanguage.googleapis.com/v1beta/interactions`, `x-goog-api-key` auth, `{"model", "input"}` body, synchronous `output_text` response) — deliberately not the two other, materially different 2026 "Gemini" surfaces also found during research: the GCP Enterprise/Vertex variant (project id + OAuth bearer tokens) and the separate, fully Google-hosted "Managed Agents" remote-sandbox product, neither of which fits ACC's local-subprocess/HTTP driver model. A `driver: 'gemini-cli'` agent type (`acc/gemini_cli.py`, mirroring `deepseek_harness.py`) runs Google's own official `gemini` CLI (`@google/gemini-cli`) headlessly: `gemini -p "<task>" --output-format json --yolo` prints one JSON object (`response`/`stats`/`error`) and exits with a documented code (0/1/42/53); `--yolo` is required for unattended tool execution, the same shape as Codex's `approval_policy: never`, so it gets the same credential-filtered environment as the other subprocess drivers. Unlike DeepAstra's `launch.py`, nothing found in gemini-cli detaches a child into its own session, so it uses ACC's ordinary process-group inheritance rather than a dedicated orphan-cleanup handler. Both new drivers ship with a fake-server/fake-executable test suite matching the existing drivers' rigor (`tests/test_gemini_adapter.py`, `tests/test_gemini_cli_adapter.py`).

The existing "Gemini/Nano Banana" entry in the capability-job provider catalog (see the top of this document) is a separate, still-unimplemented placeholder for a future image-generation feature — unrelated to these two new `kind: 'model'` workflow drivers.

Automated verification passes 153 tests, with one optional MCP SDK interoperability test skipped when its dependency is absent. Python compilation, JavaScript syntax, and shell syntax checks pass. These tests use local scripted clients, including a scripted stand-in for the Agent 3D Studio coding-agent CLI and a simulated worker claiming/finishing jobs directly against the integration queue for the job-backed-implementer path; one test (`test_agent3d.py`) drives the cooperative-cancellation mechanism end to end across real process boundaries (a real worker process, a real killed subprocess) rather than a scripted stand-in. Live provider calls, a live coding-agent run against a real reference image, RunPod GPUs, Blender, Unity, and native browser interaction remain unverified. The intermittent failure previously seen in `test_changing_reviewer_invalidates_previous_review` is root-caused and fixed: it was a test-harness race (a raw, lock-bypassing fixture write racing against `_collect()`'s still-in-flight trailing save), not a product bug — any real caller goes through a Coordinator method and already blocks on the same lock. 0/50 isolated runs and 0/5 full-suite runs failed after the fix.

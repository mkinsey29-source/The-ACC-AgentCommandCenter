...
Claude gets the same direct-API-plus-CLI pair as DeepSeek and Gemini. A `driver: 'claude'` agent type (`acc/claude_api.py`, mirroring `ollama.py`/`gemini.py`) calls Anthropic's Messages API directly over HTTP (`POST api.anthropic.com/v1/messages`, `x-api-key` + `anthropic-version: 2023-06-01` headers, `{"model", "max_tokens", "messages"}` body). Text is joined from the response's `type == "text"` content blocks; `stop_reason == "refusal"` (Anthropic's safety classifiers declining a request, HTTP 200 with no usable content) is checked explicitly rather than assumed absent. Default model is `claude-opus-5`; `thinking` is deliberately left unset rather than hard-coded, since that model runs adaptive thinking by default when the parameter is omitted, and a hard-coded config would misbehave against an older `--model` override. `driver: 'claude-code'` (`acc/claude_code.py`) wraps the actual Claude Code CLI headlessly: `claude -p "<task>" --output-format json --dangerously-skip-permissions` prints one JSON object (`type`, `subtype` — `"success"` or an `"error_*"` value — `result`, `session_id`). A real, currently-open upstream bug (anthropics/claude-code#79500) means `subtype: "success"` and exit 0 aren't fully reliable either: an API-level failure like a rate limit can come back as a "success" envelope with the real error as plain prose in `result`. This isn't specially handled — ACC's own JSON-object result contract already rejects prose (it isn't parseable JSON), so the existing `worker_prompt.extract_json_object` check catches it as a side effect rather than needing bespoke logic the way `antigravity.py`'s empty-response check did.

All four subprocess-spawning drivers (`hermes.py`, `deepseek_harness.py`, `deepastra.py`, `antigravity.py`) plus `claude_code.py` share the credential-filtered subprocess environment (`worker_prompt.subprocess_env()`).

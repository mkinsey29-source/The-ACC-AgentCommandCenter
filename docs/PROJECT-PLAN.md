...
DeepAstra is an optional starting point for the DeepSeek adapter. Resolve the previously reviewed process cleanup, prompt-delivery timeout, failure exit code, instruction loading, and Windows support issues before relying on it. Retain the small launcher concept rather than requiring the experimental visual benchmark machinery.

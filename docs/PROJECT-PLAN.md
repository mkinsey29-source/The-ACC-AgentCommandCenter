...
DeepAstra was first considered as an optional starting point for a bespoke DeepSeek launcher; ACC instead built the generic `acc/hermes.py` CLI adapter, routing DeepSeek through Hermes as one of its configured model providers rather than a standalone launcher. That adapter has since been audited against the same five concerns DeepAstra raised: process cleanup, prompt-delivery timeout, failure exit code, and instruction loading are solid (process-group inheritance reaches Hermes and its children; the prompt goes through `--query-file` rather than piped stdin, avoiding a pipe-deadlock class of bug; OS exit code and Hermes's own internal result event are both checked; the full task packet, including its instruction text, is what gets written to the query file) — a real, narrower gap was found and fixed (a mid-stream stdout failure could leave Hermes running unsupervised until ACC's own outer timeout eventually reached it; it now kills the process immediately). Windows process-tree termination remains genuinely unverified, same as always: there is no Windows host to test it on.

DeepAstra was later revisited directly: it is an unofficial, single-maintainer project (`github.com/ItsssssJack/DeepAstra`, "no affiliation or real account connection" per its own README) that wraps OpenAI's Codex CLI, reconfigured via `-c model_providers.*` overrides to call DeepSeek's (or OpenRouter's) API instead of OpenAI's. Reading its actual `launch.py` source (not tutorials) surfaced two real gaps rather than the fabricated ones found earlier for DeepSeek Harness: (1) `launch.py` spawns its `codex exec` child with `start_new_session=True`, detaching it into its own process group with no signal handler, `atexit`, or `finally` to clean it up — an external SIGTERM (what ACC's `stop_tree()` sends on stop/cancel/timeout) kills `launch.py` immediately via Python's default disposition before its own `except TimeoutExpired/KeyboardInterrupt` cleanup ever runs, orphaning a real, billed DeepSeek/OpenRouter API call; (2) `launch.py`'s `run-status.json` never carries the model's final answer text, only usage/tool-event counts, so the real answer has to be recovered from the raw Codex JSONL log it already writes under `--log-dir`. Both are fixed in `acc/deepastra.py` rather than by patching the upstream script: a `SIGTERM` handler walks `/proc/<launch.py-pid>/task/<pid>/children` to find and `killpg` the detached `codex` process directly, and the result is recovered from the log's last `item.completed` event with `item.type == 'agent_message'`, a schema confirmed against Codex's own `codex-rs/exec/src/exec_events.rs` source rather than a secondary source. The orphan-cleanup fix is proven in `tests/test_deepastra_adapter.py` against a real detached OS process (a `start_new_session=True` child that writes a heartbeat file), not a mock.

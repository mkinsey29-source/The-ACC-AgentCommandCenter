        ...
        if for_config:
            for agent in for_config:
                ...
                if driver == 'hermes':
                    ...
                    api_key_file = agent.get('api_key_file')
                    if not api_key_file:
                        raise ValueError('A claude-driver agent needs an api_key_file.')
                    model = agent.get('model', 'claude-opus-5')
                    argv = [sys.executable, str(Path(__file__).with_name('claude_api.py')),
                            '--packet', '{prompt_file}', '--api-key-file', api_key_file, '--model', model]
                    if agent.get('endpoint'):
                        argv += ['--endpoint', agent['endpoint']]
                    agent['argv'] = argv
                    ...
                    continue
                elif driver == 'claude-code':
                    executable = agent.get('executable', 'claude')
                    argv = [sys.executable, str(Path(__file__).with_name('claude_code.py')),
                            '--packet', '{prompt_file}', '--executable', executable]
                    if agent.get('model'):
                        argv += ['--model', agent['model']]
                    if agent.get('api_key_file'):

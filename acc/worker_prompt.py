...
INSTRUCTIONS = (
    ...
    'read the supplied sources from the task knowledge scopes and update the checkout note section marked '
    'ACC:CHECKOUT-SYNTHESIS with what you learned, how it applies, conflicts or uncertainty, and known mistakes '
    'to avoid. Before task work, inspect every item in Latest review queue and complete every review '
    'acknowledgement. Agree without creating a redundant review; for a conflict, state your reason and create or '
    'request a linked follow-up review. At completion include the required structured knowledge object. Put only '
    'evidence-backed learnings, validated decisions, and actual checks in the factual fields. Put reusable proven '
    'outcomes in completed_knowledge so ACC writes them to the assigned scope. Put each correction, solved or '
    'unresolved issue, unfinished item, failed loop, or workaround in its own review_items entry with situation, '
    'handling, outcome, uncertainty, and evidence. ACC routes these to separate Reviews threads. '
    'Use the exact result_contract field names. Exception arrays and review_items describe this run only; '
    'acknowledge prior review history without copying its old issues into the new check-in. '
    'Use empty arrays rather than inventing entries.\n'
